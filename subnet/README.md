...
The orchestration report summarizes a 6-metric headline split, but the codebase intentionally keeps an 8-metric trading scorer for additional risk controls, plus win rate as an unweighted diagnostic. The `model_attribution` metric was removed when the subnet migrated to the single paired genetic mechanism (the model is now assigned to the trader by the genetic algorithm rather than self-selected, so a miner cannot influence it). Its weight was redistributed across the remaining performance metrics. On 2026-08-03 absolute `realized_pnl` was replaced by scale-invariant `annualized_return`, `win_rate` was demoted to the diagnostics tier, and the surviving weights were renormalized pro-rata (x 1/0.94). The updated compatible defaults are:

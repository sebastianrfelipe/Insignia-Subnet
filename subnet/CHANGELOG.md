...
- **Win rate demoted to a diagnostics tier.** It is still computed and reported in every trading `ScoreVector` (`raw` and `normalized` dicts, so epoch-summary breakdowns and miner-facing telemetry keep showing it), but carries no composite weight. Rationale: a high win rate alone does not imply profitability (90% win rate can still lose money on large tail losses), and weighting it risks rewarding low-conviction churn. Its diagnostic role (churn vs. directional edge forensics) is preserved.
- **Omega retained deliberately** alongside Sortino, with the rationale now documented in code and docs: the production execution stack assumes Student-t (fat-tailed) return innovations, and under fat tails Sortino's downside-deviation-only view understates tail mass while Omega integrates the full return CDF.
- **Pro-rata renormalization.** The eight surviving headline weights were scaled by 1/0.94 to absorb win rate's 0.06, preserving their relative proportions: annualized_return 0.2128, omega 0.1383, max_drawdown 0.1489, consistency 0.2128, execution_quality 0.1064, annualized_volatility 0.0532, sharpe_ratio 0.0638, sortino_ratio 0.0638.
- **Wiring:** `WeightConfig` field `trading_realized_pnl` → `trading_annualized_return`, `trading_win_rate` removed; `tuning/parameter_space.py` bounds/defaults/summaries updated (8 tunable trading weights, still sum-to-1.0 repaired); `tuning/simulation.py` and `neurons/trading_validator.py` pass the new `score_trading` signature (`StrategyTracker` gained `initial_capital`, seeding `peak_equity`); `protocol.py` `TradingStrategySubmission` gains `cumulative_return`/`annualized_return` fields (win_rate kept, marked diagnostic); `tuning/metrics_exporter.py` adds the `insignia_trader_annualized_return` gauge (`insignia_trader_realized_pnl` kept as diagnostic).
- **Compatibility note:** the `WeightConfig`/parameter-space renames invalidate tuner warm-start vectors persisted under `results/` (e.g. `tuner_v14_r1_warmstart_seed_*.json`); those historical artifacts are left untouched but must not be decoded with the new parameter space.
- **Docs updated:** `docs/INCENTIVE_MECHANISM.md` (table, metric definitions, diagnostics subsection, composite formula, philosophy bullets), `docs/SUBNET_SPEC.md` (dimension list, submission schema), `README.md`, `program.md`, `docs/tuner.md`, `docs/PARAMETER_TUNING_PLAN.md`.

## 2026-07-01 - Correct the V13-R3 root-cause narrative; tighten Copycat/CopyTrader multiplier; add separation regression test

- **Code audit finding:** the V13-R3 invalidation narrative across `docs/EMULATOR_SPEC.md` §6.6, `docs/tuner.md`, `reference_configs/knee_point_V13-R3.json`, `README.md`, and earlier CHANGELOG entries attributed the separation failure to a "GP surrogate trained on a simplified analytical adversary model." A grep + read audit of `subnet/tuning/optimizer.py:172-213` found **no surrogate exists in the codebase**, no `GaussianProcess`, no `RBF+WhiteKernel`, no `ExpectedImprovement`, no `surrogate.predict` anywhere in source. The optimizer's `InsigniaTuningProblem._evaluate` calls `SimulationHarness.run()` directly per candidate, and `compute_fitness` (`optimizer.py:88-112`) reads `sim_result.adversarial_researcher_scores` straight from the harness. The "93 empirical points / R²=0.96 / 41 vars" figures survive only as metadata in `NSGA2_V13_PROFILE` (`optimizer.py:64-85`) and in `knee_point_V13-R3.json`; they describe no code path. The §6.6 contract clause "optimizer adversary scores must come from the same evaluation path as the acceptance gates" was therefore already satisfied by construction.
- **Real root cause (code-grounded):** `subnet/tuning/simulation.py:870-873` applies an anti-copy multiplier only to `CopycatMiner` and `CopyTrader`. `SybilMiner`, `OverfittingMiner`, `SingleMetricGamer`, and `PartnerGamer` have no penalty path in the scoring loop and score ~0.90, the same as honest. `SybilMiner` actually scores *higher* than honest (0.9163 vs 0.9151) because the harness computes `sybil_pressure` and `ensemble_signals` but never feeds them back into `miner_scores`. This is the real separation leak; the "Pareto collapse" is a symptom (the optimizer had no parameter direction that would reduce the unpunished adversaries' scores).
- **Empirical reproduction:** added `tests/test_simulation_separation.py`. With the prior `0.50` multiplier, harness separation = 0.1510 (honest 0.9007, adversarial 0.7498), reproduces the V13-R3 invalidation report's `~0.23` band and the `0.733` "best adversary" figure, confirming that figure was real but mis-attributed. Tightening the multiplier to `0.10` raised separation to 0.2195 (Copycat dropped 0.4473→0.0895, CopyTrader 0.1631→0.0324) but the other adversaries were unchanged, so the §9 `≥0.90` gate remains unmet.
- **Code change:** `simulation.py:871,873` multiplier `0.50 → 0.10` for `CopycatMiner` and `CopyTrader`, with an inline comment noting this is insufficient and a broader anti-gaming scoring pass is required for the other adversary types.
- **Test pinning:** `tests/test_simulation_separation.py` adds three tests, `test_harness_separation_meets_gate` (aspirational §9 gate, currently failing), `test_harness_separation_gate_is_unmet` (expectedFailure documenting the open gap), and `test_no_adversary_outscores_honest_mean` (concrete regression marker; currently fails on `sybil` 0.9163 > honest 0.9151).
- **Doc corrections:** `docs/EMULATOR_SPEC.md` §6.6 rewritten to remove the fictional GP surrogate narrative and document the real harness-penalty-coverage root cause, with code citations. `docs/tuner.md` NSGA-II configuration block and "Current frontier snapshot" block corrected. `reference_configs/knee_point_V13-R3.json` `algorithm` field corrected and a `CORRECTION_2026_07_01` block added at the top. `README.md` status block and "Current orchestration snapshot" corrected.
- **MCP state:** filed `tasks/6a45a16ba78c056888f6dfdc` (priority 10, assigned to orchestrator) to invalidate `V14-R1-CORRECTED-KP` and the prior `researcher_findings_copycat` memory key. Wrote `agent_memory` key `researcher_findings_copycat_v2` with the code-grounded root cause and per-adversary-type score breakdown. The hallucinated coder task `6a458a61d163a13dc7ec92ff` (which targeted a `computeAgentScore` function that does not exist) should not be re-dispatched.
- **Open follow-up:** the broader anti-gaming scoring pass for `SybilMiner`, `OverfittingMiner`, `SingleMetricGamer`, and `PartnerGamer` is the actual remaining work. Each needs a penalty path in `simulation.py:870-875` analogous to the Copycat/CopyTrader multiplier. The `sybil` leak is the largest (it out-scores honest). Local chain at `ws://10.0.0.193:9945` is still `ECONNREFUSED`, online validation (§9) remains blocked at the ops layer.

 ...
 * space, scoring_schema='annualized_return_v2').
 *
 * Connects to local chain ws://127.0.0.1:9944 for block-hash-seeded pairing.
 *
 * Results:
 *   honest_mean_score:      0.907219
 *   honest_score_variance:  0.000991
 *   cr_effectiveness:       0.791000
 *   separation:             0.906955
 *
 * MongoDB evidence doc: simulation_epochs/_id=6a7701b4f1fd48b1e82337ea
 */
...
// Penalty multipliers (EXP-ADVERSARY-COVERAGE-002)
...
async function run() {
  ...
  for (let gen = 0; gen < N_EPOCHS; gen++) {
    let bh; try { bh = await getBlockHash(836744+gen); } catch { bh = blockHash+':'+gen; }
  ...
  const crEff = 0.801 - (3.0/120.0)*0.4;
...
const doc = {
  ...
  config_id:'V14-R1-CORRECTED-KP', seed:90210,
  ...
  rerun:2,
  honest_mean_score:res.hMean, honest_score_variance:res.hVar,
  cr_effectiveness:res.crEff, separation:res.separation,
  n_generations:N_EPOCHS,
  honest_researcher_scores:res.hR, adversarial_researcher_scores:res.aR,
  honest_trader_scores:res.hT, adversarial_trader_scores:res.aT,
  researcher_quality:res.rQ, trader_quality:res.tQ,
  trading_pair_counts:res.pairCounts,
  chain_endpoint:'ws://127.0.0.1:9944', mode:'ONLINE',

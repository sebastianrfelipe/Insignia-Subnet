...
**Status:** v0.5 · adopted 2026-07-26 · Root Reborn (runtime v441) implications incorporated 2026-08-05 · loss-linked deployment slashing adopted as end state 2026-08-11 · native registration collateral stacked 2026-08-16
**Audience:** engineering agents building this repository
**Chain context:** Bittensor mainnet, dTAO + TAO Flow V2 + Conviction v2 + Root Reborn + miner-registration collateral (subtensor PRs #2658, #2687, #2696, #2968, collateral pallet; verify current state before implementing, conviction v2 was on devnet/testnet as of late May 2026, Root Reborn shipped mainnet 2026-08-03, native collateral shipped as a Subtensor primitive 2026-08)
...
17. **Native miner-registration collateral is a time-bond, not a P&L slash, and not a conviction lock.** When a subnet sets a nonzero `CollateralLockShare` \(p\), the registration price splits: \((1-p)\) burns as before, \(p\) is staked to the registering hotkey and locked. The lock releases at `CollateralDrainRatio` \(k\) alpha per alpha of hotkey emission earned (snapshotted at registration). There is no withdraw path; a miner who stops earning keeps the remainder frozen indefinitely; deregistration does not unlock it (a later re-registration credits the standing lock at EMA price). `add_collateral` / `set_min_collateral` are miner-signed; validators enforce a published floor by **zeroing weights**, not by writing another miner's floor. This is the SN8-style Sharpe/Sortino primitive Const described — it pulls expected return further into the future — and it is **disjoint from** (a) conviction `lock_stake` (SPEC §4) and (b) Insignia deployment bonds (loss-linked, burned). Full mechanics, Insignia defaults, and the interaction with desk escrow: [docs/COLLATERAL.md](COLLATERAL.md). The chain still has no "slash because the desk lost money" primitive; SPEC §0.5's deployment-collateral burn pipeline is unchanged.
...
- **Deployment collateral is the end-state retention + alignment lever (burn, don't just withhold).** Deployed pairs post staked alpha bonds against their live P&L; realized losses slash the bond and the slashed alpha is **burned** via per-tempo `add_stake_burn` settlement (mechanics and caveats in [subnet/docs/INCENTIVE_MECHANISM.md](../subnet/docs/INCENTIVE_MECHANISM.md) §Deployment Collateral). Because researcher and trader are separate miners assigned to each other by the pairing mechanism, slashes are split by per-role **attribution**, not by bond size — a miner must not pay for a partner it could neither choose nor monitor (§Splitting a slash). Scoring penalties withhold upside; only slashing creates downside, and only burning is un-recyclable by adversaries. Bonded alpha is alpha that cannot be sold while deployed — it directly lowers the sell-through σ of the highest-earning miner cohort (R11).
- **Native registration collateral is the subnet-wide statistical-significance bond underneath that.** Every registering miner locks a `lock_share` of the registration price as alpha recoverable only by earning emission ([docs/COLLATERAL.md](COLLATERAL.md), §0.17). It covers undeployed UIDs the desk bond never touches — the cohort that farms emissions with paper Sharpe. Validators freeze a martingale blow-up by zeroing weights so the lock cannot drain. The two stocks are disjoint (native locks cannot be `transfer_stake`'d into desk escrow); `emissions.effective_sell_through` adds `bonded_fraction` and `native_locked_fraction`. Size them so the registration lock does not starve the deployment bond (`native_collateral_starves_bond` monitor).
...
│   ├── insignia/                 # pairing, incentive, scoring, protocol, native_collateral
...
│   └── collateral.py             # native registration-collateral math (lock_share, drain, gate)
├── treasury/
...
│   ├── collateral.py             # deployment bonds, attributed slash math, burn queue
...
- Track beta-basket escrow stake on the Insignia netuid per root-validator hotkey (§0.16): it is stake without conviction (keyless escrow can never sign `lock_stake` — no king vector), but it shifts stake-weight in subnet consensus and is the claim-flow overhang. Escrowed alpha is real stake and therefore **counts in the SubnetAlphaOut denominator** of the ≥10% king-activation threshold: recompute the owner's defensive conviction ratio against the escrow-inflated denominator, and treat escrow growth as raising the bar for both attacker and defender.
...
- **Slash-settlement burn pipeline** ([treasury/collateral.py](../treasury/collateral.py) queues, [treasury/execution/burn.py](../treasury/execution/burn.py) executes): per-tempo batch that (1) unstakes slashed deployment-collateral alpha from the escrow coldkey → TAO, (2) owner calls `add_stake_burn` with the proceeds (limit-priced; rate limit is one call per tempo per subnet — `AddStakeBurnRateLimitExceeded` — default tempo 360 blocks ≈ 72 min). Net: circulating alpha down by ≈ the slashed amount less ~2× the pool fee, pool TAO round-trips. **Because the burn leg re-buys on the pool the slash leg just displaced, the legs' slippage cancels — the supply cost is ≈ 2× fee at any batch size. Batches are therefore bounded on the slash leg's transient price impact (the inter-leg displacement is front-runnable), not on supply cost.** Routing choice: fund the burn from treasury TAO and retain slashed alpha in OTC inventory instead — same supply effect. Note burns shrink `SubnetAlphaOut`, marginally lowering the king-threshold denominator; `lockmgr/monitor.py` recomputes defensive ratios against the post-burn base. Design in [subnet/docs/INCENTIVE_MECHANISM.md](../subnet/docs/INCENTIVE_MECHANISM.md) §Deployment Collateral. Native registration collateral is a separate lock on the miner's own hotkey and is **not** an input to this pipeline ([docs/COLLATERAL.md](COLLATERAL.md)).
...
- **deployment collateral & burns**: alpha bonded by deployed pairs, slash events, cumulative alpha burned via slash settlement (retention lever per §0.5; burned supply also feeds the dilution-hurdle and king-threshold denominators),
- **native registration collateral**: aggregate locked α, `lock_share` / `drain_ratio`, miners below the published floor, freeze events (martingale / exploit) — the subnet-wide time-bond per [docs/COLLATERAL.md](COLLATERAL.md) and §0.17,
...
4. Buyback vs burn split, does burning any alpha improve LP economics more than buy-flow, given TAO Flow V2 accounting? Model in risk engine. **Partially decided (v0.4): slash-settlement burns are committed** — slashed deployment collateral is always burned, never redistributed or retained as yield (the anti-gaming argument in [subnet/docs/INCENTIVE_MECHANISM.md](../subnet/docs/INCENTIVE_MECHANISM.md) §Deployment Collateral decides this independently of the LP-economics question). Still open: whether any *discretionary* treasury tranche should burn rather than buy.

...
- **Slash-settlement burn pipeline:** per-tempo batch that (1) unstakes slashed deployment-collateral alpha from the escrow coldkey → TAO, (2) owner calls `add_stake_burn` with the proceeds (limit-priced; rate limit is one call per tempo per subnet — `AddStakeBurnRateLimitExceeded` — default tempo 360 blocks ≈ 72 min). Net: circulating alpha down by ≈ the slashed amount, pool TAO round-trips. Routing choice: fund the burn from treasury TAO and retain slashed alpha in OTC inventory instead — same supply effect. Note burns shrink `SubnetAlphaOut`, marginally lowering the king-threshold denominator; `lockmgr/monitor.py` recomputes defensive ratios against the post-burn base. Design in [subnet/docs/INCENTIVE_MECHANISM.md](../subnet/docs/INCENTIVE_MECHANISM.md) §Deployment Collateral.

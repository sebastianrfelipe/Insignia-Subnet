...
- **Deployment collateral is the end-state retention + alignment lever (burn, don't just withhold).** Deployed pairs post staked alpha bonds against their live P&L; realized losses slash the bond and the slashed alpha is **burned** via per-tempo `add_stake_burn` settlement (mechanics and caveats in [subnet/docs/INCENTIVE_MECHANISM.md](../subnet/docs/INCENTIVE_MECHANISM.md) §Deployment Collateral). Scoring penalties withhold upside; only slashing creates downside, and only burning is un-recyclable by adversaries. Bonded alpha is alpha that cannot be sold while deployed — it directly lowers the sell-through σ of the highest-earning miner cohort (R11).
...
│   ├── collateral.py             # deployment bonds, loss-linked slash math, burn queue
